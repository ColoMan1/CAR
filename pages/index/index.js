  ...
  queren: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < res.data.data.data.length; i++) {
          classId1.push(res.data.data.data[i].classesId)  //把所有对应的班次id加入数组
        }
        for (var i = 0; i < res.data.data.data.length; i++) {
          routeId1.push(res.data.data.data[i].routeId)  //把所有对应的路线id加入数组
        }
        ...
        if (res.data.data.data.length == 1) {   //判断班次数量  此时无法触发班次选择change事件
          that.setData({
            ...
            classId: that.data.classId1[0],
            routeId: that.data.routeId1[0]
          })
        }
      }
    })
